...
import { useState, useEffect } from "react"
import { Star, Sparkles, Coffee, UtensilsCrossed, ShoppingCart } from "lucide-react"
import { Card, CardContent } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { categoriesAPI, menuAPI } from "@/lib/api"
...
export default function SilverStarMenu() {
  ...
  useEffect(() => {
    const sparkleArray = Array.from({ length: 20 }, () => ({
      left: `${Math.random() * 100}%`,
      top: `${Math.random() * 100}%`,
  ...
  const addToCart = (item: MenuItem) => {
    try {
      ...
      // Create unique ID including size information
      const itemId = `${item._id}-${item.sizes ? 'full' : 'regular'}`
      
      const existingItem = cart.find((cartItem: any) => cartItem.id === itemId)
      ...
      if (existingItem) {
        ...
        cart.push({
          ...
          _id: item._id,
          name: item.name,
          price: item.sizes ? item.sizes.full || item.price : item.price,
          size: item.sizes ? 'full' : undefined,
          quantity: 1,
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <Link href="/cart">
          <Button 
            variant="outline" 
            size="lg"
            className="relative bg-gradient-to-br from-amber-400/90 to-yellow-500/90 text-black font-bold rounded-full p-4 shadow-lg hover:scale-105 transition-transform"
          >
                  ...
                  <CardContent className="p-0">
                    ...
                    <div className="p-2 sm:p-3">
                      ...
                      <Button
                        ...
                        className="mt-2 w-full bg-gradient-to-r from-amber-400 to-yellow-500 text-black font-bold hover:from-amber-500 hover:to-yellow-600 text-xs sm:text-sm py-1 h-8 transition-all duration-300 transform hover:scale-[1.02]"
                      >
                        Add to Cart
                      </Button>
                    </div>
